 ...
 * @param val 增加的值。
 */
void add(vector<long long> &v, int idx, int val) {
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        long long le = sum(BIT, r); // 29) le = 左侧（已处理 i 个）中 ≤ a[i] 的个数
        ans += i - le; // 30) 本位产生的逆序对 = 左侧总数 i - ≤a[i] 个数 le（即 >a[i]）
        add(BIT, r, 1); // 31) 把当前值计入频次，为后续元素服务
    }
